...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
	...
	if (empty($nameErr) and empty($emailErr) and empty($surnameErr) and empty($phoneErr) and empty($genderErr) and empty($passwordErr) and empty($explorerErr)) {
		...
		$email = $_POST['FDemail'];
		$phone = $_POST['FDphone'];
		$bday = date('Y-m-d', strtotime(strtr($_POST['FDbday'], '/', '-')));
		...
		$query = mysqli_query($conn, "INSERT INTO form1(name,lname,email,tel,ditelindja,gjinia,paswordi,vendi,niveli,gpa,course,explorer) VALUES('$name','$surname','$email','$phone','$bday','$gender','$pass','$vendi','$level','$gpa','$course','$explorer')");
	}
}
